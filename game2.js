// JavaScript Document
/* global words, CryptoJS*/
var key = CryptoJS.enc.Hex.parse((Math.floor((Math.random() * 999999999999999999))).toString(16));
var iv = CryptoJS.enc.Hex.parse((Math.floor((Math.random() * 999999999999999999))).toString(16));

var eSplitChosenWord = words[Math.floor(Math.random() * words.length)].split("");

var eLetter1 = eSplitChosenWord[0];
var eLetter2 = CryptoJS.AES.encrypt(eSplitChosenWord[1], key, { iv: iv });
var eLetter3 = CryptoJS.AES.encrypt(eSplitChosenWord[2], key, { iv: iv });
var eLetter4 = CryptoJS.AES.encrypt(eSplitChosenWord[3], key, { iv: iv });
var eLetter5 = CryptoJS.AES.encrypt(eSplitChosenWord[4], key, { iv: iv });

eSplitChosenWord = [eLetter1, eLetter2, eLetter3, eLetter4, eLetter5];
var eChosenWord = eSplitChosenWord.join("");
...
table.onkeyup = function (e) {
	...
	if (target === rows[usingRowNumber][0]) {
		rows[usingRowNumber][0].value = eLetter1;
		
	}
};

tryRow1Letter1Input.value = eLetter1;
tryRow2Letter1Input.value = eLetter1;
tryRow3Letter1Input.value = eLetter1;
tryRow4Letter1Input.value = eLetter1;
tryRow5Letter1Input.value = eLetter1;
...
function combineAttemptLettersAndCompareWithChosenWord() {
	attempt = eLetter1 + CryptoJS.AES.encrypt(rows[usingRowNumber][1].value.toLowerCase(), key, { iv: iv }) + CryptoJS.AES.encrypt(rows[usingRowNumber][2].value.toLowerCase(), key, { iv: iv }) + CryptoJS.AES.encrypt(rows[usingRowNumber][3].value.toLowerCase(), key, { iv: iv }) + CryptoJS.AES.encrypt(rows[usingRowNumber][4].value.toLowerCase(), key, { iv: iv });
	
	if (attempt === eChosenWord) {
		for (var i = 0; i <= 4; i++) {
